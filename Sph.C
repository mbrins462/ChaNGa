...
#ifdef COOLING_NUC
	treeProxy.updateuDot(0, duDelta, dStartTime, param.bGasCooling, 1, 1,
			     CkCallbackResumeThread());
#else
...
void Main::initCooling()
{
    ...
    if(!bIsRestarting) {  // meaning not restarting from a checkpoint.
	    ...
	    if(arrayFileExists(basefilename + ".helium", nTotalParticles)) {
		CkPrintf("Reading helium file\n");
		HeFourOutputParams pHeFourOut(basefilename, 0, 0.0);
		treeProxy.readTipsyArray(pHeFourOut, CkCallbackResumeThread());
		}
	    if(arrayFileExists(basefilename + ".carbon", nTotalParticles)) {
		CkPrintf("Reading carbon file\n");
		CTwelveOutputParams pCTwelveOut(basefilename, 0, 0.0);
		treeProxy.readTipsyArray(pCTwelveOut, CkCallbackResumeThread());
		}
	    if(arrayFileExists(basefilename + ".oxygen", nTotalParticles)) {
		CkPrintf("Reading oxygen file\n");
		OSixteenOutputParams pOSixteenOut(basefilename, 0, 0.0);
		treeProxy.readTipsyArray(pOSixteenOut, CkCallbackResumeThread());
		}
	    if(arrayFileExists(basefilename + ".neon", nTotalParticles)) {
		CkPrintf("Reading neon file\n");
		NeTwentyOutputParams pNeTwentyOut(basefilename, 0, 0.0);
		treeProxy.readTipsyArray(pNeTwentyOut, CkCallbackResumeThread());
		}
	    if(arrayFileExists(basefilename + ".magnesium", nTotalParticles)) {
		CkPrintf("Reading magnesium file\n");
		MgTwentyFourOutputParams pMgTwentyFourOut(basefilename, 0, 0.0);
		treeProxy.readTipsyArray(pMgTwentyFourOut, CkCallbackResumeThread());
		}
	    if(arrayFileExists(basefilename + ".silicon", nTotalParticles)) {
		CkPrintf("Reading silicon file\n");
		SiTwentyEightOutputParams pSiTwentyEightOut(basefilename, 0, 0.0);
		treeProxy.readTipsyArray(pSiTwentyEightOut, CkCallbackResumeThread());
		}
	    if(arrayFileExists(basefilename + ".sulfur", nTotalParticles)) {
		CkPrintf("Reading sulfur file\n");
		SThirtyTwoOutputParams pSThirtyTwoOut(basefilename, 0, 0.0);
		treeProxy.readTipsyArray(pSThirtyTwoOut, CkCallbackResumeThread());
		}
	    if(arrayFileExists(basefilename + ".argon", nTotalParticles)) {
		CkPrintf("Reading argon file\n");
		ArThirtySixOutputParams pArThirtySixOut(basefilename, 0, 0.0);
		treeProxy.readTipsyArray(pArThirtySixOut, CkCallbackResumeThread());
		}
	    if(arrayFileExists(basefilename + ".calcium", nTotalParticles)) {
		CkPrintf("Reading calcium file\n");
		CaFortyOutputParams pCaFortyOut(basefilename, 0, 0.0);
		treeProxy.readTipsyArray(pCaFortyOut, CkCallbackResumeThread());
		}
	    if(arrayFileExists(basefilename + ".titanium", nTotalParticles)) {
		CkPrintf("Reading titanium file\n");
		TiFortyFourOutputParams pTiFortyFourOut(basefilename, 0, 0.0);
		treeProxy.readTipsyArray(pTiFortyFourOut, CkCallbackResumeThread());
		}
	    if(arrayFileExists(basefilename + ".chromium", nTotalParticles)) {
		CkPrintf("Reading chromium file\n");
		CrFortyEightOutputParams pCrFortyEightOut(basefilename, 0, 0.0);
		treeProxy.readTipsyArray(pCrFortyEightOut, CkCallbackResumeThread());
		}
	    if(arrayFileExists(basefilename + ".iron", nTotalParticles)) {
		CkPrintf("Reading iron file\n");
		FeFiftyTwoOutputParams pFeFiftyTwoOut(basefilename, 0, 0.0);
		treeProxy.readTipsyArray(pFeFiftyTwoOut, CkCallbackResumeThread());
		}
	    if(arrayFileExists(basefilename + ".nickel", nTotalParticles)) {
		CkPrintf("Reading nickel file\n");
		NiFiftySixOutputParams pNiFiftySixOut(basefilename, 0, 0.0);
		treeProxy.readTipsyArray(pNiFiftySixOut, CkCallbackResumeThread());
		}
            }
        }
#endif
                              ...
                              const CkCallback& cb)
{
	...
	if (p->isGas()) {
	    ...
	    T = p->u() / dTuFac;
	    int particleNumber = p->iOrder;//MB
	    //printf("Sph.C RestartEnergy: Particle Number = %i, Temperature = %e\n",particleNumber, T); //MB
            PERBARYON Y;
            CoolPARTICLEtoPERBARYON(cl, &Y, &p->CoolParticle());
#if defined( COOLING_HELM) || defined( COOLING_NUC)            
	    p->u() = clThermalEnergy(&p->CoolParticle(), p->fDensity,T)*cl->diErgPerGmUnit;
#else
			   ...
			   const CkCallback& cb)
{
    ...
    for(unsigned int i = 1; i <= myNumParticles; ++i) {
	GravityParticle *p = &myParticles[i];
	//if (TYPETest(p, TYPE_GAS) && p->rung >= activeRung) { // I belive this is a mistake MRB
	if (TYPETest(p, TYPE_GAS) ) {
	    double T,E;
#ifndef COOLING_NONE
	    int particleNumber = p->iOrder;//MB
	    T = p->u() / dTuFac;//MB: Energy in Code Units ==> Temperature in Kelvin?
	    //printf("Sph.C InitEnergy: (Input values) Particle Number = %i, p->u() = %e, dTuFac = %e, Temperature[K?] = %e\n", particleNumber, p->u(), dTuFac, T);//MB
	    ...
	    COOLPARTICLE *cp;
	    cp = &p->CoolParticle();
	    //printf("Sph.C InitEnergy: (After CoolInitEnergyAndParticleData) Particle Number = %i, heFour = %e\n", particleNumber, cp->HeFour);// MB
	    //printf("Sph.C InitEnergy: (After CoolInitEnergyAndParticleData) Particle Number = %i,  p->u() = %e, p->u() [erg/gm] = %e\n", particleNumber, E, E*cl->dErgPerGmUnit);//MB
	    //printf("Sph.C InitEnergy: (Input density for CoolCodeEnergyToTemperature) Density = %e\n", p->fDensity);//MB
            double temp = CoolCodeEnergyToTemperature( cl, &p->CoolParticle(), p->u(), p->fDensity, p->fMetals());//MB
	    //printf("Sph.C InitEnergy: (After CoolCodeEnergyToTemperature) Particle Number = %i, Temperature = %e\n", particleNumber, temp);//MB
#endif
	    p->uPred() = p->u();
