...
#ifdef COOLING_NUC
		      //printf("TreePiece.cpp kick: heFour frac = %e\n", p->HeFour());//MB
		      //printf("TreePiece.cpp kick: heFourDot = %e\n", p->HeFourDot());//MB
		      p->HeFour() += p->HeFourDot()*duDelta[p->rung];
		      p->HeFourPred() = p->HeFour();
		      //printf("TreePiece.cpp kick: HeFourPred = %e\n", p->HeFourPred());//MB
		      p->CTwelve() += p->CTwelveDot()*duDelta[p->rung];
		      p->CTwelvePred() = p->CTwelve();
		      p->OSixteen() += p->OSixteenDot()*duDelta[p->rung];
		      p->OSixteenPred() = p->OSixteen();
		      p->NeTwenty() += p->NeTwentyDot()*duDelta[p->rung];
		      p->NeTwentyPred() = p->NeTwenty();
		      p->MgTwentyFour() += p->MgTwentyFourDot()*duDelta[p->rung];
		      p->MgTwentyFourPred() = p->MgTwentyFour();
		      p->SiTwentyEight() += p->SiTwentyEightDot()*duDelta[p->rung];
		      p->SiTwentyEightPred() = p->SiTwentyEight();
		      p->SThirtyTwo() += p->SThirtyTwoDot()*duDelta[p->rung];
		      p->SThirtyTwoPred() = p->SThirtyTwo();
		      p->ArThirtySix() += p->ArThirtySixDot()*duDelta[p->rung];
		      p->ArThirtySixPred() = p->ArThirtySix();
		      p->CaForty() += p->CaFortyDot()*duDelta[p->rung];
		      p->CaFortyPred() = p->CaForty();
		      p->TiFortyFour() += p->TiFortyFourDot()*duDelta[p->rung];
		      p->TiFortyFourPred() = p->TiFortyFour();
		      p->CrFortyEight() += p->CrFortyEightDot()*duDelta[p->rung];
		      p->CrFortyEightPred() = p->CrFortyEight();
		      p->FeFiftyTwo() += p->FeFiftyTwoDot()*duDelta[p->rung];
		      p->FeFiftyTwoPred() = p->FeFiftyTwo();
		      p->NiFiftySix() += p->NiFiftySixDot()*duDelta[p->rung];
		      p->NiFiftySixPred() = p->NiFiftySix();
#endif
#else /* COOLING_NONE */
...
#ifdef COOLING_NUC
              // FIXME Do we need a different time step
              // for nuclear burning?
	      p->HeFourPred() += p->HeFourDot()*duDelta;
	      p->CTwelvePred() += p->CTwelveDot()*duDelta;
	      p->OSixteenPred() += p->OSixteenDot()*duDelta;
	      p->NeTwentyPred() += p->NeTwentyDot()*duDelta;
	      p->MgTwentyFourPred() += p->MgTwentyFourDot()*duDelta;
	      p->SiTwentyEightPred() += p->SiTwentyEightDot()*duDelta;
	      p->SThirtyTwoPred() += p->SThirtyTwoDot()*duDelta;
	      p->ArThirtySixPred() += p->ArThirtySixDot()*duDelta;
	      p->CaFortyPred() += p->CaFortyDot()*duDelta;
	      p->TiFortyFourPred() += p->TiFortyFourDot()*duDelta;
	      p->CrFortyEightPred() += p->CrFortyEightDot()*duDelta;
	      p->FeFiftyTwoPred() += p->FeFiftyTwoDot()*duDelta;
	      p->NiFiftySixPred() += p->NiFiftySixDot()*duDelta;
#endif
#else
