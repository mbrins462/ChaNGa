  ...
  if(bParaWrite) {
    ...
    for(unsigned int i = 1; i <= myNumParticles; ++i) {
      ...
      if(params.bFloat) {
	      ...
	      //MB debug: print out values of silicon (after testing, this prints the correct values)
	      //if(params.sTipsyExt == "silicon") {printf("InOutput.C TreePiece::outputAscii: silicon = %.14g\n", dOut);}
	     }
          }
      else
